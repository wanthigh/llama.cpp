...
class device_info {
 public:
  ...
  int get_max_compute_units() const {
    return _max_compute_units;
  }
  int get_max_work_group_size() const {
    return _max_work_group_size;
  }
  int get_max_sub_group_size() const {
    return _max_sub_group_size;
  }
  ...
  template <
      typename NDRangeSizeTy = size_t*,
      std::enable_if_t<
          std::is_same_v<NDRangeSizeTy, size_t*> ||
...
class device_ext : public sycl::device {
 ...
 public:
  ...
  int is_native_atomic_supported() {
    return 0;
  }
  int get_major_version() const {
...
#warning "Querying the number of bytes of free memory is not supported"
  ...
  sycl::queue* create_in_order_queue(bool enable_exception_handler = false) {
    std::lock_guard<mutex_type> lock(m_mutex);
    return create_queue_impl(
        enable_exception_handler, sycl::property::queue::in_order());
  }

  sycl::queue* create_in_order_queue(
      sycl::context context,
      sycl::device device,
      bool enable_exception_handler = false) {
    std::lock_guard<mutex_type> lock(m_mutex);
...
#if defined(__linux__)
    mapped_address_space = (byte_t*)mmap(
        nullptr,
        mapped_region_size,
        PROT_NONE,
...
static inline void* dpct_malloc(
    size_t& pitch,
    size_t x,
    size_t y,
    size_t z,
...
static inline sycl::event dpct_memset(
    sycl::queue& q,
    void* dev_ptr,
    valueT value,
    size_t size) {
 ...
 */
template <typename valueT>
static inline std::vector<sycl::event> dpct_memset(
    sycl::queue& q,
    pitched_data data,
    valueT value,
    sycl::range<3> size) {
...
static inline std::vector<sycl::event> dpct_memset(
    ...
    size_t y) {
  return dpct_memset(
      q, pitched_data(ptr, pitch, x, 1), val, sycl::range<3>(x, y, 1));
}

static memcpy_direction deduce_memcpy_direction(
    sycl::queue& q,
    void* to_ptr,
    const void* from_ptr,
  ...
  switch (dir) {
    ...
    case memcpy_direction::automatic: {
      ...
      return direction_table[static_cast<unsigned>(get_pointer_attribute(
          q,
          to_ptr))][static_cast<unsigned>(get_pointer_attribute(q, from_ptr))];
    }
    default:
...
static inline size_t get_copy_range(
    sycl::range<3> size,
    size_t slice,
    size_t pitch) {
...
static inline std::vector<sycl::event> dpct_memcpy(
  ...
  switch (direction) {
    case host_to_host:
      for (size_t z = 0; z < size.get(2); ++z) {
        ...
        if (to_range.get(0) == from_range.get(0) &&
          ...
          for (size_t y = 0; y < size.get(1); ++y) {
            event_list.push_back(dpct_memcpy(
                q, to_ptr, from_ptr, size.get(0), direction, dep_events));
    ...
    case host_to_device: {
      host_buffer buf(
          get_copy_range(size, to_slice, to_range.get(0)), q, event_list);
    ...
    case device_to_host: {
      host_buffer buf(
          get_copy_range(size, from_slice, from_range.get(0)), q, event_list);
...
static inline std::vector<sycl::event> dpct_memcpy(
  ...
  return dpct_memcpy(
      ...
      sycl::range<3>(from.get_pitch(), from.get_y(), 1),
      to_id,
      from_id,
      size,
      direction);
...
inline typename DataType<T>::T2 get_value(const T* s, sycl::queue& q) {
  ...
  if (get_pointer_attribute(q, s) == pointer_access_attribute::device_only)
    detail::dpct_memcpy(
        q, (void*)&s_h, (const void*)s, sizeof(T), device_to_host)
        .wait();
  else
...
template <typename VecT, class BinaryOperation>
inline unsigned vectorized_binary(
    unsigned a,
    unsigned b,
...
T permute_sub_group_by_xor(
  ...
  return sycl::select_from_group(
      g,
      x,
      target_offset < logical_sub_group_size ? start_index + target_offset
                                             : id);
}
...
static inline void async_dpct_memcpy(
    void* to_ptr,
    size_t to_pitch,
    const void* from_ptr,
    size_t from_pitch,
  ...
  detail::dpct_memcpy(
      q, to_ptr, from_ptr, to_pitch, from_pitch, x, y, direction);
}
...
class device_memory {
 ...
 private:
  ...
  void allocate_device(sycl::queue& q) {
    ...
    if (Memory == shared) {
      _device_ptr =
          (value_t*)sycl::malloc_shared(_size, q.get_device(), q.get_context());
      return;
    }
