...
template <int vdr>
static __dpct_inline__ float vec_dot_q4_0_q8_1_impl(
    const int* v,
    const int* u,
    const float& d4,
...
template <int vdr>
static __dpct_inline__ float vec_dot_q5_0_q8_1_impl(
    const int* vl,
    const int* vh,
    const int* u,
    const float& d5,
...
#pragma unroll
  for (int i = 0; i < vdr; ++i) {
    int vi0 =
        (vl[i] >> 0) & 0x0F0F0F0F; // lower 4 qs bits, still need qh as 5th bits
    ...
    int vi1 =
        (vl[i] >> 4) & 0x0F0F0F0F; // upper 4 qs bits, still need qh as 5th bits
...
template <int vdr>
static __dpct_inline__ float vec_dot_q5_1_q8_1_impl(
    const int* vl,
    const int* vh,
    const int* u,
...
template <int vdr>
static __dpct_inline__ float vec_dot_q8_0_q8_1_impl(
    const int* v,
    const int* u,
    const float& d8_0,
    const float& d8_1) {
...
template <int vdr>
static __dpct_inline__ float vec_dot_q8_1_q8_1_impl(
    const int* v,
    const int* u,
...
#pragma unroll
  for (int i = 0; i < QR4_K; ++i) {
    ...
    const int dot1 = dpct::dp4a(
        v1i,
        u[2 * i + 1],
        dpct::dp4a(v0i, u[2 * i + 0], 0)); // SIMD dot product
    const int dot2 = dpct::dp4a(
        0x01010101,
        u[2 * i + 1],
...
#pragma unroll
  ...
  return vec_dot_q5_0_q8_1_impl<VDR_Q5_0_Q8_1_MMVQ>(
      vl, vh, u, bq5_0->d, bq8_1->ds);
}
...
#pragma unroll
  ...
  return vec_dot_q5_1_q8_1_impl<VDR_Q5_1_Q8_1_MMVQ>(
      vl, vh, u, bq5_1->dm, bq8_1->ds);
}
...
#pragma unroll
  ...
  return vec_dot_q8_0_q8_1_impl<VDR_Q8_0_Q8_1_MMVQ>(
      v, u, bq8_0->d, bq8_1->ds[0]);
}
...
static __dpct_inline__ float vec_dot_q4_K_q8_1(
  ...
  const int dot1 =
      dpct::dp4a(ui2, v2 & 0x0f0f0f0f, dpct::dp4a(ui1, v1 & 0x0f0f0f0f, 0));
  const int dot2 = dpct::dp4a(
      ui4, (v2 >> 4) & 0x0f0f0f0f, dpct::dp4a(ui3, (v1 >> 4) & 0x0f0f0f0f, 0));
...
#pragma unroll
  ...
  const float sumf_d =
      d8_1 * (dpct::dp4a(ui1, v1, 0) * s[0] + dpct::dp4a(ui2, v2, 0) * s[1]) +
...
static __dpct_inline__ float vec_dot_iq2_xxs_q8_1(
  ...
  const float d =
      (float)bq2->d * (0.5f + (aux32 >> 28)) * bq8_1[ib32].ds[0] * 0.25f;
...
static __dpct_inline__ float vec_dot_iq3_xxs_q8_1(
    const void* __restrict__ vbq,
    const block_q8_1* __restrict__ bq8_1,
    const int& iqs,
...
static __dpct_inline__ float vec_dot_iq3_s_q8_1(
    const void* __restrict__ vbq,
    const block_q8_1* __restrict__ bq8_1,
    const int& iqs,
  ...
  for (int l = 0; l < 4; ++l) {
    const uint32_t* grid1 =
        iq3s_grid + (qs[2 * l + 0] | ((bq2->qh[ib32] << (8 - 2 * l)) & 256));
    const uint32_t* grid2 =
        iq3s_grid + (qs[2 * l + 1] | ((bq2->qh[ib32] << (7 - 2 * l)) & 256));
...
static __dpct_inline__ float vec_dot_iq1_s_q8_1(
    const void* __restrict__ vbq,
    const block_q8_1* __restrict__ bq8_1,
    const int& iqs,
